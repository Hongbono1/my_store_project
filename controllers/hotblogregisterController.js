...
export async function registerHotBlog(req, res) {
  try {
    ...
    res.json({ success:true, id: result.rows[0].id, message:"홍보 블로그 저장 완료" });
  } catch (err) {
    console.error("registerHotBlog error:", err);
    res.status(500).json({ success:false, error:"DB insert failed", detail: err?.message });
  }
}
