...
export async function saveBannerSlot(req, res) {
    try {
        ...
        if (file) {
            // static: app.use("/uploads", express.static("public/uploads"));
            // → /uploads/adminAds/파일명 형태로 접근
            imageUrl = `/uploads/adminAds/${file.filename}`;
        }
        ...
        return res
            .status(500)
            .json({ ok: false, message: "배너 저장 중 서버 오류", error: err.message });
    }
}
...
export async function saveTextSlot(req, res) {
    try {
        ...
        return res
            .status(500)
            .json({ ok: false, message: "텍스트 저장 중 서버 오류", error: err.message });
    }
}

/**
 * 배너 슬롯 조회
 * GET /manager/ad/slot?page=index&position=xxx
 */
export async function getSlot(req, res) {
    try {
        ...
        const sql = `
            SELECT id, page, position, slot_type, image_url, link_url, text_content, created_at, updated_at
            FROM admin_ad_slots
...
export async function getTextSlot(req, res) {
    try {
        ...
        const sql = `
            SELECT id, page, position, text_content as content, created_at, updated_at
            FROM admin_ad_slots
