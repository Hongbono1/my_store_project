...
export async function getFoodStoresByCategory(req, res) {
  ...
  const sub = String(req.query.sub || "").trim(); // ✅ 추가
  ...
  try {
    // ✅ 동적 조건 구성 (sub 있으면 detail_category까지 필터)
    const params = [category];
    let where = `
      WHERE s.business_type = '음식점'
        AND s.business_category = $1
    `;

    if (sub) {
      params.push(sub);
      where += `
        AND NULLIF(TRIM(COALESCE(s.detail_category,'')), '') = $2
      `;
    }
    ...
    const stores = result.rows.map((r) => ({
      ...
      detail_category: r.detail_category || "", // ✅ 프론트 필터/표시용
